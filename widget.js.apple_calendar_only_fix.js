(() => {
  ...
  async function boot_() {
    ...
    const theme = {
      ...
      booking_url: String(config.booking_url || "").trim(),
      defaultUrl: String((config.booking_url || "")).trim(),
    };
  ...
  function renderServicePicker_(ui, theme, state) {
    ...
    theme.services.forEach(svc => {
      ...
      b.onclick = () => {
        ...
        // Calendar-only fix: resolve URL robustly without changing UI
        const url = resolveBookingUrl_(svc, theme);

        // If service explicitly wants request flow, honor it
        const wantsRequest = String(svc.request_fallback || "").toLowerCase() === "true" || svc.request_fallback === true;

        if (url && !wantsRequest) {
          svc.booking_url = url; // normalize for downstream iframe
  ...
  // Resolve booking URL for a service robustly (keeps UI unchanged)
  function resolveBookingUrl_(svc, theme) {
    try {
      ...
      const candidates = [
        ...
        s.url,
        s.link,
        // fallbacks from config (some backends supply these)
        theme.booking_url,
        theme.defaultUrl
      ];
